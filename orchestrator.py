...
def run(args):
    ...
    digest = plans.plan_hash(bases, args.sieve_limit, args.seed, args.max_digits, candidates)
...
def parse_args(argv=None):
    ...
    parser.add_argument("--max-candidates", type=int, help="limit the ordered GPU candidate plan")
    ...
    if args.max_candidates is not None and args.write_candidate_plan:
        parser.error("--max-candidates cannot be combined with --write-candidate-plan")
