...
__global__ void single_block_arbitrary_conditional_sub_p_kernel(uint64_t* r, const uint64_t* p, size_t d, size_t n) {
    ...
    for (int iter = 0; iter < 2; iter++) {
        ...
        for (int chunk = num_chunks - 1; chunk >= 0; chunk--) {
            ...
            if (tid == 0) { shared_max_idx = -1; shared_max_sign = 0; }
            __syncthreads();
